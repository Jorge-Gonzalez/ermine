...
const cases: Case[] = [
  ...
  { s: "attach-below-xs attach-left", expect: "ok", why: "scale-backed block offset composes with a left edge pin" },
  { s: "center-y inset-right-sm", expect: "ok", why: "vertical centering composes with a right inset" },
  { s: "attach-below attach-below-xs", expect: "fail", why: "two values on the same top edge relation" },
  { s: "attach-left stretch-inline", expect: "fail", why: "left edge pin overlaps inline stretch" },
  { s: "attach-below-xxs", expect: "fail", why: "offset edge attachment is restricted to spacing steps" },
  { s: "attach-right inset-right-sm", expect: "fail", why: "two values on the same right edge relation" },
  { s: "cover center-y", expect: "fail", why: "cover owns the top edge relation" },
  { s: "cover inset-right-sm", expect: "fail", why: "cover owns every edge relation" },
  { s: "inset-right-xxs", expect: "fail", why: "positioned insets are restricted to spacing steps" },
