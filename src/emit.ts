...
const EMISSION: Record<string, EmitSpec> = {
  ...
  // --- affordance: the interaction cursor cue (R-SKIN-17). ---
  affordance: {
    effectKind: "css",
    plain: (word) => {
      switch (word) {
        case "pressable": return { cursor: "pointer" };
        case "blocked": return { cursor: "not-allowed" };
        default: return null;
      }
    },
  },
  ...
  elevation: {
    ...
    plain: (word) => {
      switch (word) {
        ...
        case "recessed-soft":
          return { "box-shadow": "var(--shadow-recessed-soft, inset 0 1px 3px rgb(0 0 0 / 28%))" };
        case "recessed-fail":
