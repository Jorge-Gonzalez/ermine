...
- **Current assimilation:** the generated current ledger now reports **512 current
  declarations**, **228 adopted/infrastructure declarations**, **284 project-owned
...
The current generated ledger is at Monky `986115c`: 512 declarations, 228
adopted/infrastructure declarations, 284 project-owned residue declarations,
...
Phase D's gate holds the list at zero. The current gate at Monky `986115c` reports 512
current declarations, 284 project-owned residue, `assimilable = 0`, and no shadowed words;
...
`hug-inline`, and `center-x`. The remaining Phase 3 grammar work is the proportional/relational plane
(`center-y` / flow-center, `measure`, viewport fill, grid fit, and columns + intent-proportions) plus the smaller
...
counts at that checkpoint; later `fill`, `square`, `cover`, `push`, `hug-inline`, and `center-x` migrations
moved the live ledger to 512 current declarations, 284 project-owned residue, `assimilable = 0`,
and `shadowed = 0`.
